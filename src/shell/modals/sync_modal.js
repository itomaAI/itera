...
(function(global) {
    ...
    class SyncModal {
        ...
        _updateStatusUI(status, details = null) {
            ...
            const map = {
                'idle': { text: '● Standby', color: 'text-text-muted' },
                'syncing': { text: '↻ Syncing...', color: 'text-warning animate-pulse' },
                'synced': { text: '✓ Synced Successfully', color: 'text-success' },
                'error': { text: '⚠️ Sync Error', color: 'text-error' }
            };
        ...
        async _handleAuthToggle() {
            if (this._isAuthenticated()) {
                ...
                const redirectUri = window.location.origin + window.location.pathname; // 現在のURL
                const scope = 'https://www.googleapis.com/auth/drive.file'; // 自身が作成したファイルのみアクセス可能な安全なスコープ
                ...
                // 2. ユーザーにトークンを手動でコピペさせるダイアログを表示（セキュリティと実装の簡易化のため）
                const token = prompt(`Please complete the Google authentication in the new window.\n\nAfter granting access, you will be redirected. Look at the URL bar of the new window, copy the string after 'access_token=', and paste it here:`);
                
                if (token && token.trim() !== "") {
                    // トークンの保存 (VFSではなくHost側のlocalStorageに保存)
                    let secrets = {};
                    try { secrets = JSON.parse(localStorage.getItem('itera_sync_secrets') || '{}'); } catch(e) {}
